...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     tot_BMN = 0.
      ...
      !!by zhang print out soil water
      !!===============================
       do 5 k=1, sol_nly(ihru) 
       
              T_NO3         =T_NO3 + sol_no3(k,ihru)
              T_NH3         =T_NH3 + sol_nh3(k,ihru)          
...
!!    added for binary files 3/25/09 gsm line below and write (33333
     ...
     !! temperarily set  to zero
          ...
          write (1001,9001) iyr, i_mo, i, j, rsdc_d(j), sedc_d(j), percc_d(j),             &
              ...
              tot_LSN,tot_LMN,tot_HSN,tot_HPN,tot_BMN,                              &
              bio_ms(j)*0.42, rwt(j), tot_no3_nh3,wdntl,etday,tillage_factor(j),    &
              (soilwater(ii), ii = 1, 11), (wfsc(ii), ii = 1, 11),                   &
        !!      T_CFMETS1,T_CFSTRS1,T_CFSTRS2,T_CFS1S2,T_CFS1S3,T_CFS2S1,T_CFS2S3,T_CFS3S1, &
        !!    T_EFS1S2,T_EFMETS1,T_EFSTRS1,T_EFSTRS2,T_EFS1S3,T_EFS2S1,T_EFS2S3,T_EFS3S1,                             &
        !!    T_IMMMETS1,T_IMMSTRS1,T_IMMSTRS2,T_IMMS1S2,T_IMMS1S3,T_IMMS2S1,                                         &
        !!    T_IMMS2S3,T_IMMS3S1,T_MNRMETS1,T_MNRSTRS1,T_MNRSTRS2,T_MNRS1S2,                                         &
        !!    T_MNRS1S3,T_MNRS2S1,T_MNRS2S3,T_MNRS3S1,T_CO2FMET,T_CO2FSTR_L,                                          &
