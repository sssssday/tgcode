  subroutine carbon_zhang2
      ...
      !!===========================================
       do k = 1, sol_nly(j)
          ...
          !!if (sol_tmp(k,j) > 0. .AND. sol_st(k,j) > 0.) then !! qichun this is the original limit
     ...
     !! qichun code from century ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      !! qichun code from century~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~       
		      ...
		      CS = 0.
		    CS=MIN(10.,SQRT(cdg*sut)*0.9*OX*X1) !! qichun this is the original 
		   !!CS=MIN(10.,SQRT(t_scale*w_scale)*0.9*OX*X1)  !! qichun
		    !!CS=MIN(10.,SQRT(tcalc *wcalc )*0.9*OX*X1)
		   !! CS = t_scale*w_scale  !! qichun
		      !!   
		      
		      if (k == 1) then 
		        combined_factor(j) = CS
		      end if
        ...
        !  	      WNO3(ISL)=WNO3(ISL)+RNMN(ISL)
           ...
           21 DF1=LSNTA
              ...
              rspc_d(j) = rspc_d(j) +  sol_RSPC(k,j) 
                     ...
                     !! move denitrification here by Qichun
!! `~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    
               ...
               !! call denitrification (to use void and cdg factor)
                wdn = 0.
                cdg = fcgd(sol_tmp(k,j))
                !!call ndenit(k,j,cdg,wdn,void, sol_RSPC(k,j)) !! qichun 
              if (cdg > 0. ) then 
              !!if (cdg > 0. .and. void <= 0.1) then
                  call ndenit(k,j,cdg,wdn,void, sol_RSPC(k,j)) !! qichun 
                end if
                  !! check if n2oprod is updated correctly?
