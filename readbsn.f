...
!     if (ievent == 1) nstep = 24
 ...
 !!Parameter variables added D. Moriasi 4/8/2014  
  ...
  !!----------------------------------------------------------------        
      ...
      if (cdn_bsn <= 0.) cdn_bsn = 1.4 !! original is 1.4
...
!!    at some future time)
     ...
     &'sol_solp' 
      ...
      open (1001,file="cswat_daily.txt")
       write (1001,5104)'year','month','day','hru','rsdc','sedc',
     &'percc','latc','emitc','grainc','surfq_c',
     &'stoverc','NPPC','foc','rspc','tot_mass','tot_cmass','tot_nmass',
     &'tot_LSC','tot_LMC','tot_HSC','tot_HPC','tot_BMC','tot_LSN',
     &'tot_LMN','tot_HSN','tot_HPN','tot_BMN',
     &'Biom_C','rwtf',
     &'tot_no3_nh3','wdntl', 
     ...
     &'WFSC9','WFSC10','WFSC11',
   !!  & 'CFMETS1','CFSTRS1',
   !!  & 'CFSTRS2','CFS1S2','CFS1S3','CFS2S1','CFS2S3','CFS3S1','EFS1S2',
   !!  &'EFMETS1','EFSTRS1','EFSTRS2','EFS1S3','EFS2S1','EFS2S3','EFS3S1',
   !!  & 'IMMMETS1','IMMSTRS1','IMMSTRS2','IMMS1S2','IMMS1S3','IMMS2S1',
   !!  & 'IMMS2S3','IMMS3S1','MNRMETS1','MNRSTRS1','MNRSTRS2','MNRS1S2',
   !!  & 'MNRS1S3','MNRS2S1','MNRS2S3','MNRS3S1','CO2FMET','CO2FSTR_L',
